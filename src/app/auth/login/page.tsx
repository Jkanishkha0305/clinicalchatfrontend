...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success('Successfully logged in!', { title: 'Welcome Back' });
      router.push('/');
    } catch (error) {
      ...
      // Backend errors show as toast (not field-specific)
    ...
    <div className="min-h-screen flex justify-center items-center p-4 bg-gradient-to-br from-[#f0f4f8] via-[#e8f0f7] to-[#d6e9f7]">
      <div className="bg-white rounded-2xl shadow-[0_20px_40px_rgba(13,71,161,0.12)] w-full max-w-[400px] overflow-hidden border border-[rgba(13,71,161,0.08)]">
        ...
        <div className="px-8 pt-8 pb-6 text-center">
          <h1 className="text-2xl font-bold text-[#0d47a1] mb-2">Welcome Back</h1>
          <p className="text-[#546e7a] text-sm">Sign in to continue exploring clinical trial insights.</p>
        </div>

        {/* Form Section */}
        <div className="px-8 pb-6">
          <form method="POST" action="/api/auth/login" className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
            <div className="flex flex-col gap-2">
              <label htmlFor="login-username" className="text-sm font-semibold text-[#1e293b]">
                Username
              </label>
              <input 
                id="login-username" 
            ...
            <div className="flex flex-col gap-2">
              ...
              />
            {validationErrors.password && (
              <p className="text-xs text-red-600 mt-1 ml-1" data-error-for="password">
                {validationErrors.password}
              </p>
            )}
              </div>
        ...
        <div className="px-8 pb-8 pt-4 border-t border-gray-100 bg-[#fafbfc]">
          <div className="flex justify-center items-center gap-2 text-sm text-[#607d8b]">
            <span>New here?</span>
            <a href="/auth/signup" className="text-[#1976d2] font-semibold hover:text-[#1565c0] hover:underline transition-colors">
              Create an account
            </a>
          </div>
        </div>
