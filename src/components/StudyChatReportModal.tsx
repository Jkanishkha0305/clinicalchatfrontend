...
interface StudyChatReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  studyId: string;
...
export default function StudyChatReportModal({ isOpen, onClose, studyId, chatSessionId }: StudyChatReportModalProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await searchApi.generateStudyChatReport(studyId, chatSessionId, format, abortController);
      ...
      if (response.success) {
        ...
        if (!abortController?.signal.aborted && chatSessionId) {
          try {
            const sessionResponse = await chatSessionsApi.get(chatSessionId);
          ...
          toast.success('Study chat report generated and saved!');
        }
      } else {
        toast.error('Failed to generate study chat report');
      }
    } catch (error) {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Failed to generate study chat report'
      );
      }
    } finally {
      setLoading(false);
  ...
  const handlePrint = () => {
    ...
    if (printWindow && report) {
      printWindow.document.write(`
        ...
        <html>
          <head>
            <title>Study Chat Research Report</title>
            <style>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!report ? (
          <div>
            <h2 style={{ color: '#667eea', marginBottom: '20px' }}>AI Study Chat Report Generator</h2>
            <p style={{ color: '#666', marginBottom: '20px' }}>
              Generate a comprehensive research report based on your chat conversation about this specific clinical trial study.
            </p>
            
            <div style={{ marginBottom: '20px', padding: '15px', background: '#f0f4ff', borderRadius: '8px', border: '1px solid #667eea' }}>
              <p style={{ margin: '5px 0', fontWeight: '600', color: '#333' }}>
